...
export function GameCard({ game, rank }: { game: GameCardData; rank?: number }) {
  return <article className="game-card">
    ...
    <div className="game-card-body">
      <div className="game-card-top"><div><Link href={`/games/${game.slug}`} title={game.title}><h3>{game.title}</h3></Link><p>{game.developer}</p></div><BookmarkButton slug={game.slug}/></div>
